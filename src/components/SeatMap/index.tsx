...
import { wagonCapacity } from '@/lib/format/seat-prefs';
...
export function SeatMap({
  ...
  const handleSeatClick = (seat: number) => (): void => onToggle(seat);
        ...
        <div className="flex items-center gap-1">
          ...
          <div className="flex gap-1">
            {Array.from({ length: columns }, (_, i) => {
              const col = i + 1;
              return (
                <div key={col} className={`flex flex-col gap-1 ${col % 2 === 0 ? 'mr-2' : ''}`}>
                  {renderSeat(2 * col)}
                  {renderSeat(2 * col - 1)}
                </div>
              );
