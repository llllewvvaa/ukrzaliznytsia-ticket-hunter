...
import type { TimetableTrain } from './timetable';
...
describe('trip-format', () => {
  ...
  it('departLabel joins boarding departure with arrival time', () => {
    const t: TimetableTrain = {
      number: '66',
      route: 'Київ Суми',
      periodicity: [],
      boardings: [{ station: 'Київ', departTime: '22:10' }],
      arriveStation: 'Суми',
      arriveTime: '05:25',
    };
    expect(departLabel(t)).toBe('22:10 → 05:25');
    expect(departLabel({ ...t, boardings: [] })).toBe('05:25');
    expect(departLabel({ ...t, arriveTime: '' })).toBe('22:10');
    expect(departLabel({ ...t, boardings: [], arriveTime: '' })).toBe('');
  });
});
