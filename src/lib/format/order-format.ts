import type { OrderTicket, OrderTicketReservation, Station, UserOrder } from '@/lib/models';
...
export function durationLabel(
  departSec: number | undefined,
  arriveSec: number | undefined,
): string {
...
export function orderTotal(order: UserOrder): number {
  return (order.tickets ?? []).reduce((sum, t) => sum + (Number(t.reservation?.price) || 0), 0);
}
