...
describe('scheduler: pure helpers', () => {
  ...
  it('monitorDelayMs honors an active backoff window', () => {
    ...
    expect(
      monitorDelayMs(makeJob({ pollIntervalMs: 8_000, pausedUntil: now + 25_000 }), now),
    ).toBe(25_000);
    // expired backoff → normal interval
    expect(
      monitorDelayMs(makeJob({ pollIntervalMs: 8_000, pausedUntil: now - 1 }), now),
    ).toBe(8_000);
  });

  it('warmupAtMs is startAt minus the lead, or null', () => {
    expect(warmupAtMs(makeJob({ startAt: undefined }))).toBeNull();
    expect(warmupAtMs(makeJob({ startAt: 5_000_000 }))).toBe(
      5_000_000 - SPRINT_WARMUP_LEAD_MS,
    );
  });
