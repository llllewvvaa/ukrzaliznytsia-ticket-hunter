...
import { debugEnabled, recordApiRequest } from '@/lib/ui/debug';
...
export async function rawRequest(path: string, opts: RequestOpts = {}): Promise<Response> {
...
export function getPassengers(signal?: AbortSignal): Promise<Passenger[]> {
  return requestJson<unknown>('/v2/passengers', signal ? { signal } : {}).then(parsePassengers);
}

export function getTrip(tripId: number, signal?: AbortSignal): Promise<Trip> {
  return requestJson<unknown>(`/v3/trips/${tripId}`, signal ? { signal } : {}).then(parseTrip);
}
...
export function searchTrips(params: SearchTripsParams, signal?: AbortSignal): Promise<Trip[]> {
...
export function getArchivedOrders(page = 1, signal?: AbortSignal): Promise<ArchivedOrdersResponse> {
  return requestJson<unknown>(`/v2/orders/archived?page=${page}`, signal ? { signal } : {}).then(
    parseArchivedOrders,
  );
}
...
export function parseWagons(raw: unknown): Wagon[] {
  ...
  const list = Array.isArray(data?.wagons) ? data.wagons : Array.isArray(data) ? data : null;
