...
function scan(value: unknown, found: Found, parentKey = '', depth = 0): void {
  ...
  for (const [rawKey, v] of Object.entries(value as Record<string, unknown>)) {
    ...
    if (found.userId === undefined) {
      ...
      const isUserId = key === 'userid' || (key === 'id' && USER_CONTEXT_RE.test(parent));
      if (isUserId) {
...
function isComplete(found: Found): found is { token: string; sessionId: string; userId: number } {
  return found.token !== undefined && found.sessionId !== undefined && found.userId !== undefined;
}
