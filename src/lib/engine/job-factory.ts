import { newJobId } from '@/lib/store';
...
import type { CoachType, HuntJob, JobMode, MatchRef, SeatPrefs, Station } from '@/lib/models';
...
export function createJobDraft(input: JobFormInput, now: number = Date.now()): JobDraftResult {
  const errors: string[] = [];
  ...
  const name = input.name?.trim() || `${input.from.name} → ${input.to.name} (${input.date})`;
