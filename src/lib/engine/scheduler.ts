...
export function rateLimitPauseUntil(
  ...
  const wait = Math.max(pollIntervalMs, (retryAfterSec ?? 0) * 1_000, RATE_LIMIT_MIN_PAUSE_MS);
...
export async function scheduleMonitorAlarm(job: HuntJob, now = Date.now()): Promise<void> {
  await browser.alarms.create(tickAlarmName(job.id), {
    when: now + monitorDelayMs(job, now),
  });
}

export async function scheduleSprintWarmup(job: HuntJob, now = Date.now()): Promise<void> {
