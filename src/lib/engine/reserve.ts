...
import { diagnoseRelaxations, selectSeats, type SeatChoice } from '@/lib/format/seat-prefs';
  ...
  try {
    ...
    try {
      const ready = await pollCart(cartId, {
        ...opts.pollOptions,
        onPending: (retryIn) => emit({ status: 'cart_pending', cartId, retryInSec: retryIn }),
      });
      return finish({
        status: 'reserve_ok',
        cartId,
        ...(ready.payment_url ? { paymentUrl: ready.payment_url } : {}),
        ...(typeof ready.expire_at === 'number' ? { reservedUntil: ready.expire_at * 1000 } : {}),
      });
    } catch (err) {
      if (err instanceof RateLimitError) {
        return finish({
