...
vi.mock('@/lib/api/auth', () => ({ getAuthHeaders: vi.fn() }));
...
import { searchTrips, parseSearchTrips, RateLimitError, AuthError } from '@/lib/api/uz-api';
import {
  matchTrip,
  huntOnce,
  startJob,
...
import type { HuntJob, Trip, UzAuthHeaders } from '@/lib/models';
...
function makeTrip(number: string, classes: Array<{ id: string; free: number }>): Trip {
  return {
    id: 3000001,
    depart_at: 0,
    arrive_at: 0,
...
describe('matchTrip', () => {
  ...
  it('filters by preferred train numbers', () => {
    ...
    expect(matchTrip([makeTrip('091К', [{ id: 'П', free: 9 }])], job)?.trip.train.number).toBe(
      '091К',
    );
  });
  ...
  it('matches a timetable number (no zero-pad/letter) against booking.uz format', () => {
    ...
    expect(matchTrip([makeTrip('066Ш', [{ id: 'К', free: 4 }])], job)?.trip.train.number).toBe(
      '066Ш',
    );
...
describe('huntOnce', () => {
  ...
  it('stays hunting and counts the attempt on no match', async () => {
    ...
    vi.mocked(searchTrips).mockResolvedValue([makeTrip('057К', [{ id: 'П', free: 0 }])]);
  ...
  it('transitions to reserving and dispatches the match', async () => {
    ...
    vi.mocked(searchTrips).mockResolvedValue([makeTrip('057К', [{ id: 'П', free: 3 }])]);
  ...
  it('skips when another job holds the reserve lock', async () => {
    ...
    vi.mocked(searchTrips).mockResolvedValue([makeTrip('057К', [{ id: 'П', free: 9 }])]);
