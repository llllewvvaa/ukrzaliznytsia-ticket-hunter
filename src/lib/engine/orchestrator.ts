...
import { deleteJob, getJob, listJobs, patchJob } from '@/lib/store';
import { log } from '@/lib/ui/logger';
import { getAuthHeaders } from '@/lib/api/auth';
import type { JobControlMessage } from '@/lib/messages';
import {
  AuthError,
  NotDiscoveredError,
  RateLimitError,
...
async function handleHuntError(job: HuntJob, err: unknown): Promise<void> {
  ...
  if (err instanceof RateLimitError) {
    const { pollIntervalMs, pausedUntil } = rateLimitPauseUntil(job, err.retryAfterSec);
...
async function pause(job: HuntJob, reason: PauseReason, detail?: string): Promise<void> {
  await clearJobAlarms(job.id);
  releaseReserveLock(job.id);
  await patchJob(job.id, {
    state: 'paused',
