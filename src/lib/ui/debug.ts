...
import { DEBUG_EVENT } from '@/lib/messages';
...
export function startPageDebugBridge(): void {
  ...
  window.addEventListener('message', (e: MessageEvent) => {
    ...
    if (d?.__uzDebug === true && d.event && enabled) {
      void browser.runtime.sendMessage({ type: DEBUG_EVENT, event: d.event }).catch(() => {});
    }
  });
...
const SECRET_HEADER_RE =
  /^(authorization|proxy-authorization|cookie|set-cookie|x-auth-token|x-api-key)$/i;
