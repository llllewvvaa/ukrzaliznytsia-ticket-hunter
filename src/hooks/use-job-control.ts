...
import { controlConfirmCopy } from '@/lib/format/job-format';
...
export type JobView = { kind: 'list' } | { kind: 'new' } | { kind: 'detail'; jobId: string };
...
export function useJobControl() {
  ...
  const createAndStart = useCallback(async (job: HuntJob, onDone?: () => void): Promise<void> => {
    try {
      await saveJob(job);
