...
`chrome.storage.session` and builds headers. `uz-api.ts` is context-agnostic: callers wire
in a header provider + 401 handler via `configureApi`. In the SW that provider reads the
cached session (and invalidates on 401); in the content script it reads the page session live.
